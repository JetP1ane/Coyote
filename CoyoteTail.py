# coding=utf-8
######################################################################
##|# ------ CoyoteTail : Coyote logging and output class ----- #|###
...
class CoyoteTail :
	...
	def mangleException(self, pkt, reason=''):
		self.notifyBad('\nCoyote PANIC : Process failed during MANGLING', 1, 1)
		if reason != '':
			self.notifyBad('Reason : ' + reason, 1)
		self.notify('Packet was logged to errorLogFile : Coyote.err', 1)
		logfd = open('Coyote.err', 'a')
		logfd.write(
			'---DUMP BEGINS--------------------------------------------------------------------------------------\n')
		logfd.write(
	...
	## CoyotePanic : unrecoverable exception handling ##
	def CoyotePanic(self, msg, bold=1, exitOnFailure=1) :
		if bold == 1 :
			msg = '\033[1m' + 'Coyote PANIC : ' + msg + '\033[0m'
		else :
			msg = 'Coyote PANIC : ' + msg
		if exitOnFailure == 1 :
			exit(msg)
		else :
			print(msg)
