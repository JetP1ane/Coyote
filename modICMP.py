...
class modICMP :
	...
	def __init__(self, ip_host, ip_rogue, mac_host, mac_rogue, debugLevel=1) :
		...
		self.CoyoteTail.notify('Loading ICMP module...', 1)
		self.ICMPthreads = []
	...
	def Coyote_Control_Message_Protocol(self, ICMPpkt) :
		if ICMPpkt[ICMP].type in self.requestTypes :  # request
			self.CoyoteTail.notify('ICMP request received', 3)
	...
	def createICMPthread(self, pkt) :
		...
		self.CoyoteTail.notify('New ICMPthread created', 3)
	...
	def deleteICMPthread(self, ICMPthread) :
		try :
			...
			self.ICMPthread_number -= 1
			self.CoyoteTail.notify('ICMPthread deleted', 3)
			return True
		except ValueError :
			self.CoyoteTail.CoyotePanic('Unexpected exception was raised during deletion of ICMPthread')
	...
	def ICMPRequestMangling(self, pkt) :
		if pkt[Ether].src == self.mrogue :  # packet from rogue
			...
			self.CoyoteTail.notify('ICMP Request from host to network - FORWARD', 2)
			return pkt
		else :
			self.CoyoteTail.CoyotePanic('NOT YET IMPLEMENTED (ICMP request from network)')
	...
	def ICMPReplyMangling(self, pkt, ICMPthread) :
		if ICMPthread.src_mac == self.mrogue :  # packet for rogue
			...
			self.CoyoteTail.notify('ICMP Request from network to host - FORWARD', 2)
			return pkt
		else :
			self.CoyoteTail.CoyotePanic('NOT YET IMPLEMENTED (ICMP reply from network)')


	## Rewrites ICMP packets ##
	def pktRewriter(self, pkt, src, dst, msrc, mdst) :
		self.CoyoteTail.notify('ICMP packet is being rewritten :', 3)
		if src != 0 :
		...
		self.CoyoteTail.notify('ICMP packet mangled and rewritten successfully', 3)
		return pkt
