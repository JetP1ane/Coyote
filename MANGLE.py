...
class MANGLE:
	def __init__(self, ip_host, ip_rogue, mac_host, mac_rogue, debugLevel=1):
		self.banner()
		print('\n')
		print('\033[1m\033[32m[*] COYOTE is waking up...\033[0m')
		self.CoyoteTail = CoyoteTail(debugLevel)
		self.CoyoteTail.notify('Loading CoyoteTail...', 1)
		...
		self.PKTthreads = []
		self.pktNumber = 0
		self.threshold = self.CoyoteTail.threshold
		self.PKTthread_number = 0
		self.PKTthread_index = 0
		self.FILTER = Coyote_Internal_Light_Trafic_Efficient_Ruling(debugLevel)
		...
		self.CoyoteTail.notifyGood('COYOTE init complete ! Ready to process trafic...\n', 1, 1)

	## NAT LOGIC ##
	def Coyote_Address_Translation(self, pkt):
		self.CoyoteTail.notify('\033[7m--- ProcessPKT ---\033[27m', 3)
		self.pktNumber += 1
		if 'TCP' in pkt or 'UDP' in pkt:
			self.CoyoteTail.notify('IP packet received. Entering IP processing routines...', 3)
			if self.FILTER.FILTER_routine(pkt):  # Coyote's Internal Light-Traffic Efficient Ruling
				current_PKTthread = self.PKTthread_exist(pkt)
				#current_PKTthread.gatherSeqNum(pkt)
				IPpkt = self.Coyote_Mangling(pkt, current_PKTthread)
				self.CoyoteTail.notify('IP packet sent', 3)
				self.closeConn_sniff(pkt, current_PKTthread)
				return IPpkt
			else:
				self.CoyoteTail.notify('Packet got dropped by FILTER', 2)
				self.CoyoteTail.notify(str(ls(pkt)), 3)
				return False
		# Else, we may need some exceptional processing (ARP stuff, etc...)
		else:
			self.CoyoteTail.notify('Non-TCP/UDP packet received. Entering special processing routines...', 3)
			if 'ARP' in pkt:
				ARPpkt = self.modARP.Coyote_Address_Resolution_Protocol(pkt)
				self.CoyoteTail.notify('ARP packet sent', 3)
				return ARPpkt
			elif 'ICMP' in pkt:
				ICMPpkt = self.modICMP.Coyote_Control_Message_Protocol(pkt)
				self.CoyoteTail.notify('ICMP packet sent', 3)
				return ICMPpkt
			elif 'EAPOL' in pkt:
				self.CoyoteTail.notify('EAPOL packet sent', 3)
				return pkt.__class__(raw(pkt))
			self.CoyoteTail.notify('No special packet handlers found... Forwarding packets', 3)
			return pkt
	...
	def PKTthread_exist(self, pkt):
		for PKTthread in self.PKTthreads:
			if PKTthread.thisIsMyPKT(pkt) == True:
				self.CoyoteTail.notify('PKTthread exists...', 2)
				return PKTthread
		self.CoyoteTail.notify('New PKTthread detected. Creation...', 2)
	...
	def create_PKTthread_from_reverseCon(self, pkt):
		...
		self.CoyoteTail.notify('PKTthread created from reverse connection', 3)
	...
	def create_PKTthread_from_bindCon(self, pkt):
		...
		self.PKTthreads.append(PKTthreadInstance)
		self.PKTthread_number += 1
		self.CoyoteTail.notify('PKTthread created from bind connection', 3)
		return PKTthreadInstance
	...
	def deletePKTthread(self, PKTthread):
		try:
			...
			self.PKTthread_number -= 1
			self.CoyoteTail.notify('PKTthread deleted', 3)
			return True
		except:
			self.CoyoteTail.CoyotePanic('Unexpected exception was raised during deletion of PKTthread : PKTthread does not exist (this can happen...)', 0, 0)
			return True

	## Mangling manager ##
	def Coyote_Mangling(self, pkt, PKTthread):
		if pkt[IP].src == self.host:  # Packet for network on a host-network Pthread, no need for mangling
			self.CoyoteTail.notify('Packet from host to network - FORWARD', 2)
			return pkt
		elif pkt[IP].dst == self.host and PKTthread.src_ip == pkt[IP].dst: # Packet for host on a host-network Pthread, no need for mangling
			self.CoyoteTail.notify('Packet from network to host - FORWARD', 2)
			return pkt
			...
			#self.CoyoteTail.CoyotePanic('Unknown situation while matching PKTthread',0,0)
			return pkt

	## Rewrite packet IPs and MACs ##
	def pktRewriter(self, pkt, src, dst, msrc, mdst):
		self.CoyoteTail.notify('IP packet is being rewritten :', 3)
		if pkt[IP].src != src:
			self.CoyoteTail.notify('\t' + pkt[IP].src + ' --> ' + src, 3)
			pkt[IP].src = src
		if pkt[IP].dst != dst:
			self.CoyoteTail.notify('\t' + pkt[IP].dst + ' --> ' + dst, 3)
			pkt[IP].dst = dst
		if pkt[Ether].src != msrc:
			self.CoyoteTail.notify('\t' + pkt[Ether].src + ' --> ' + msrc, 3)
			pkt[Ether].src = msrc
		if pkt[Ether].dst != mdst:
			self.CoyoteTail.notify('\t' + pkt[Ether].dst + ' --> ' + mdst, 3)
			pkt[Ether].dst = mdst
		...
		self.CoyoteTail.notify('IP packet mangled and rewritten successfully', 3)
		return pkt
	...
	def closeConn_sniff(self, pkt, PKTthread):
		...
		if 'TCP' in pkt:
			if pkt[TCP].flags & RST:
				self.CoyoteTail.notifyWarn('RST packet went through. Deleting associated PKTthread...', 3)
				self.deletePKTthread(PKTthread)
				return True
			elif pkt[TCP].flags & FIN:
				self.CoyoteTail.notify('FIN packet went through. Checking PKTthread state...', 3)
				if PKTthread.state == 'active':
					PKTthread.changeState('FIN_sent')
					self.CoyoteTail.notify('\tPKTthread state changed to  "FIN_sent"', 3)
					return False
				elif PKTthread.state == 'FIN_sent':
					PKTthread.changeState('FIN_acknowledged')
					self.CoyoteTail.notify('\tPKTthread state changed to "FIN_acknowledged"', 3)
					return False
				elif PKTthread.state == 'FIN_acknowledged':
					self.CoyoteTail.CoyotePanic(
					...
					self.CoyoteTail.CoyotePanic('PKTthread is in unknown state : ' + pkt)
					exit()
			elif (pkt[TCP].flags & ACK) and PKTthread.state == 'FIN_acknowledged':
				self.CoyoteTail.notify('ACK packet went through for a terminating PKTthread. Checking PKTthread state...', 3)
				PKTthread.changeState('zombie')
				self.CoyoteTail.notifyWarn('\tPKTthread state changed to "zombie"', 3)
				self.deletePKTthread(PKTthread)
	...
	def banner(self) :
		...
		print("                                           o8'         aP'")
		print("                                            YaaaP'    ba")
		print("                           \033[31mCOYOTE\033[0m\033[1m         Y8'         88")
		print("                                       ,8\"           `P")
		print("                                  ,d8P'              ba")
...
#				self.gatherTCPSessID2(pkt)
	...
	def gatherTCPSessID1(self, pkt) :
		if 'TCP' in pkt:
			...
			exit("Coyote PANIC : non TCP packet was sent to gatherTCPSessID. This should not happen")
...
########################################################################
### --- FILTER : Coyote's Internal Light-Trafic Efficient Ruling --- ###
######## --- Class implementing some Coyote-level filtering --- ########
########################################################################

### THIS CLASS IS DECOMMISSIONNED !!! ###
class Coyote_Internal_Light_Trafic_Efficient_Ruling():
	actions = ['drop', 'log']

	def __init__(self, debugHerited, rulesToLoad=[], specialRulesToLoad=[], filename='Coyote.log'):
		self.CoyoteTail = CoyoteTail(debugHerited)
		self.enabled = False
		self.CoyoteTail.notify('Loading FILTER module...', 1)
		self.logfile = filename
		self.rules = rulesToLoad
		self.specialRules = []
		if len(rulesToLoad) > 0 or len(specialRulesToLoad) > 0:
			self.CoyoteTail.notify('\tRule(s) loaded successfully (' + str(rulesToLoad) + str(specialRulesToLoad) + ' rule(s))', 2)
		else:
			self.CoyoteTail.notify('\tNo rule to load', 2)
			self.enabled = False
		...
		if not self.enabled:
			...
			for rule in self.specialRules:
				if rule[0] == pkt.sport or rule[0] == pkt.dport:
					self.CoyoteTail.notify('Special rule applied (port : ' + rule[2] + ') [Packet DROPPED]', 3)
					return False
			for rule in self.rules:
				if self.applyRule(rule, pkt):
					return False
			return True
	...
	def applyRule(self, rule, pkt):
		if pkt[IP].src == rule[0] or pkt[IP].dst == rule[0]:
			if pkt.sport == rule[1] or pkt.dport == rule[1]:
				action = rule[3]
				self.CoyoteTail.notify('Rule applied (port : ' + rule[1] + ', action : ' + rule[2] + ')', 3)
				return action()
	...
	def ruleAdd(self, host, port, action='drop'):
		...
		self.CoyoteTail.notify('Rule added', 3)
